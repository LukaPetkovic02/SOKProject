def generate_graph(graph):
    # Function to determine which function to call based on the type of graph
    if graph.directed:
...
def generate_directed_graph(graph):
    ...
    for edge in graph.edges.values():
        ...
        if edge.node1 != edge.node2:
            ...
            if not reverse_edge_exists:
                ...
                graph_string += "{source: '" + str(edge.node1.id) + "', target: '" + str(edge.node2.id) + ", curved: true'},"
    ...
    graph_string += """
        ...
        var simulation = d3.forceSimulation(nodes.concat(selfLoops))
            ...
            .force("charge", d3.forceManyBody().strength(-20)) // Set a low repulsion force
...
def generate_undirected_graph(graph):
    # Start the string with the JavaScript code to create the graph
    graph_string = "<script>"
    graph_string += "var nodes = ["
